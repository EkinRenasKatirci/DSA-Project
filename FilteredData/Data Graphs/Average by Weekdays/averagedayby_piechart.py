...
end_date = datetime(2024, 11, 27, tzinfo=timezone_offset)
...
for txt_file in txt_files:
    ...
    try:
        ...
        weekday_avg.plot(
            ...
            colormap='Pastel1',  # Optional color map
            legend=True,
            ylabel="",  # Remove the default y-axis label
            title=f"Weekday Proportions (25 Sep - 27 Nov)\n{txt_file}"
        )
        
        # Save the plot as an image in the same folder
        plot_path = os.path.join(folder_path, f"{os.path.splitext(txt_file)[0]}_weekday_avg_filtered_pie.png")
        plt.tight_layout()
        plt.savefig(plot_path)
        plt.close()  # Close the figure to save memory
        
        print(f"Pie chart saved for {txt_file} at {plot_path}")
