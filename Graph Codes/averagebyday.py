...
for txt_file in txt_files:
    ...
    try:
        ...
        plt.figure()
        weekday_avg.plot(kind='line', marker='o', color='skyblue', linewidth=2)
        ...
        plot_path = os.path.join(folder_path, f"{os.path.splitext(txt_file)[0]}_weekday_avg_filtered_line.png")
        plt.tight_layout()
        plt.savefig(plot_path)
        plt.close()  # Close the figure to save memory
        
        print(f"Line graph saved for {txt_file} at {plot_path}")
